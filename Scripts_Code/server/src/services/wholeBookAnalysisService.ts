...
import { getProjectPaths, getProject } from './projectService.js';
import { chunkManuscript, cleanupChunks, toProjectRelative, type ChunkInfo } from './bookChunkerService.js';
...
const OUTPUT_FILES: Record<AnalysisType, string> = {
  plot:      'MASTER_PLOT_CHARACTER_GUIDE.md',
  character: 'CHARACTER_VOICE_DIALOGUE_GUIDE.md',
  voice:     'authorial_voice_analysis.md',
};

// ─── System prompts ───────────────────────────────────────────────────────────

const PLOT_SYSTEM_PROMPT = `You are building THE MASTER PLOT & NARRATIVE GUIDE for a novel. You process the manuscript in sequential chunks, iteratively building this reference document.
...
• SYMBOL APPEARANCES section at document end — log every recurring symbol/motif occurrence (note distinctive imagery, sensory signatures, recurring objects/numbers/colors)
...
const CHARACTER_SYSTEM_PROMPT = `You are building THE CHARACTER VOICE & DIALOGUE GUIDE for a novel. You process the manuscript in sequential chunks, iteratively building this reference document.
...
• Physical Appearance: cite the specific scene where each detail appears
...
const VOICE_SYSTEM_PROMPT = `You are building THE STYLE TRANSFER MASTER PROFILE for a novel — an engineering specification designed to enable an LLM to replicate this author's prose voice with mechanical precision.
...
interface ChunkMessageParams {
  index: number; total: number; charStart: number; charEnd: number;
  chunkPath: string; outputDocPath: string; manuscriptName: string;
  bookTitle: string; runDate: string; isFirst: boolean;
}

function buildPlotChunkMessage(p: ChunkMessageParams): string {
...
function buildCharacterChunkMessage(p: ChunkMessageParams): string {
  const header = `CHUNK ${p.index} of ${p.total} | chars ${p.charStart.toLocaleString()}–${p.charEnd.toLocaleString()}
...
interface VoiceChunkParams extends ChunkMessageParams {
  sampleLabel: string;
  sampleNum: number;
}

function buildVoiceChunkMessage(p: VoiceChunkParams): string {
...
// ─── State file helpers (per-project) ────────────────────────────────────────

function getStateFilePath(projectId: string): string {
  return path.join(getProjectPaths(projectId).aiOutput, 'analysis_chunked_state.json');
}

function getChunksDir(projectId: string): string {
  return path.join(getProjectPaths(projectId).aiOutput, '.chunks');
}

async function readState(projectId: string): Promise<PipelineStatus | null> {
  try {
    const raw = await fs.readFile(getStateFilePath(projectId), 'utf-8');
...
async function writeState(projectId: string, state: PipelineStatus): Promise<void> {
  await fs.writeFile(getStateFilePath(projectId), JSON.stringify(state, null, 2), 'utf-8');
}

// ─── Cancellation handle (one active pipeline per server) ─────────────────────
...
export async function getChunkedAnalysisStatus(projectId: string): Promise<PipelineStatus | null> {
  return readState(projectId);
}

// ─── Resolve manuscript path for a project ────────────────────────────────────

async function resolveManuscriptPath(projectId: string): Promise<{ absPath: string; name: string }> {
  const project = await getProject(projectId);
  const paths = getProjectPaths(projectId);

  // Preferred: project.json's rawFile field
  if (project.rawFile) {
    const candidate = path.join(paths.rawFiles, project.rawFile);
    try {
      await fs.access(candidate);
      return { absPath: candidate, name: project.rawFile };
    } catch { /* fall through */ }
  }

  // Fallback: first .txt in RawFiles/
  try {
    const files = await fs.readdir(paths.rawFiles);
    const txtFiles = files.filter(f => f.endsWith('.txt')).sort();
    if (txtFiles.length > 0) {
      return { absPath: path.join(paths.rawFiles, txtFiles[0]), name: txtFiles[0] };
    }
  } catch { /* no RawFiles dir */ }

  throw new Error(`No raw manuscript file found for project ${projectId}. Place a .txt file in the project's RawFiles/ directory.`);
}
...
export async function runPipeline(
  ...
  const { absPath: manuscriptAbsPath, name: manuscriptName } = await resolveManuscriptPath(projectId);

  let completedTypes: string[] = [];
  if (options.resume) {
    const existing = await readState(projectId);
  ...
  await fs.mkdir(paths.aiOutput, { recursive: true });

  // Chunk the manuscript
  const chunksDir = getChunksDir(projectId);
  let chunks: ChunkInfo[];
  try {
    chunks = await chunkManuscript(manuscriptAbsPath, chunksDir, options.chunkSize ?? 40_000);
  } catch (err: any) {
    onEvent({ type: 'error', error: `Failed to chunk manuscript: ${err.message}`, recoverable: false });
    return;
  }

  // Convert project-relative chunk paths (the chunker produces these correctly)
  ...
  await writeState(projectId, state);
  ...
  for (const analysisType of typesToRun) {
    ...
    const outputDocAbsPath = path.join(paths.aiOutput, OUTPUT_FILES[analysisType]);
    const outputDocProjectPath = toProjectRelative(outputDocAbsPath);
    ...
    for (const { chunk, sampleLabel, sampleNum } of chunksForType) {
      ...
      const baseParams: ChunkMessageParams = {
        index: chunk.index, total: chunk.total,
        charStart: chunk.charStart, charEnd: chunk.charEnd,
        chunkPath: chunk.projectRelPath,
        outputDocPath: outputDocProjectPath,
        manuscriptName,
        bookTitle,
        runDate,
        isFirst,
      ...
      if (analysisType === 'plot') {
        userMessage = buildPlotChunkMessage(baseParams);
      } else if (analysisType === 'character') {
        userMessage = buildCharacterChunkMessage(baseParams);
      } else {
        userMessage = buildVoiceChunkMessage({ ...baseParams, sampleLabel, sampleNum });
      }

      try {
        await callClaudeForChunk(systemPrompt, userMessage, modelId, onEvent, abort.signal);
